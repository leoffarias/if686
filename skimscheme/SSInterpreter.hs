                  ...
                  case (Map.lookup func env) of
                      ...
                      otherwise ->
                        (stateLookup env func >>= \res ->
                          case res of
                            (Closure xis@(List (Atom "lambda" : List formals : body:l)) envClosure) ->
                              ST $ (\s -> let envs = union envClosure env
                                              ...
                                              (ST f2) = define novaClosure [Atom func, List [Atom "make-closure", xis]] --POE EVAL AQUI
                                              (resultInutil, envZika) = f2 $ union env1 $ union env s  --mesma coisa do da aplicação de f1, mas agora considerando o env resultante de f1
                                              teuKu = union (difference envZika (difference envClosure env)) env -- atualiza o env com os itens específicos da clausura q foram atualizados no envZika. Sempre dando preferencia pro env encontrado mais recentemente (zica, depois o de clausura, depois o geralzão)
                                in (result1, teuKu)
                              )
